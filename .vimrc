...
"窗口参数设置:
winpos 5 5                                                  "设定窗口位置
set lines=65 columns=220                                    "设定窗口大小
...
"tab/backspace相关设置:
set expandtab                                               "将 tab 化为 space 存储
set shiftwidth=4                                            "设置 tab 宽度为4个 space
set tabstop=4                                               "将 tab 解释为4个空格
set softtabstop=4                                           "在编辑动作(backspace)时，对空白的解释(在操作backspace时，4个空白会被当成一个tab删除)
...
set fencs=utf-8,gbk,big5,cp936,gb18030,gb2312,utf-16,ucs-bom,shift-jis	
                                                            "打开文件时进行解码的猜测列表
